...
function suscribirEventoDescarGaCSV() {
    $("#btnDescargaCsv, #btnDescargaCsv1").click(function (e) {
        window.location.href = "/TemplateOKDOC.csv";        
    });
}
    ...
    try {
        ...
        if (data.length > 0) {
            for (n in data) {
                ...
                if (data[n].OKDOC.toString().trim() === "") { data[n].OKDOC = 0 };

                if (!isNaN(parseInt(data[n].OKDOC.toString().trim()))) {
                    if (parseInt(data[n].OKDOC.toString().trim()) === 0 || parseInt(data[n].OKDOC.toString().trim()) === 1) {
                        ...
                        tmpData[n].OK = parseInt(data[n].OKDOC.toString().trim());
