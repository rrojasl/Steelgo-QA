...
function AjaxCargarRevision() {
    ...
    loadingStart();
    $DynasolCamion.Camion.read({ token: Cookies.get("token"), OrdenCompraID: $("#inputOrdenCompra").data("kendoComboBox").value(), NombreCamion: $("#InputCamion").val().trim() }).done(function (data) {
        if (Error(data)) {
    ...
    try {
        ...
        for (index = 0; index < arregloCaptura.length; index++) {
            if (arregloCaptura[index].Agregar || arregloCaptura[index].Accion == 3) {
                ...
                ListaDetalles[cont] = {
                    Accion: "", DetalleCamionID: "", OrdenCompraID: "", ColadaID: "", Cantidad: "", CamionID: "", Estatus: 1
                };

                ListaDetalles[cont].Accion = arregloCaptura[index].Accion;
                ListaDetalles[cont].DetalleCamionID = arregloCaptura[index].DetalleCamionID;
                ...
                ListaDetalles[cont].CamionID = arregloCaptura[index].CamionID;
                ListaDetalles[cont].Cantidad = arregloCaptura[index].CantCamion;


                if ((arregloCaptura[index].CantCamion == "" || arregloCaptura[index].CantCamion == 0) && arregloCaptura[index].Accion != 3) {
...
function AjaxEjecutarGuardado(Captura, tipoGuardar) {
    $DynasolCamion.Camion.create(Captura, { OrdenCompraID: $("#inputOrdenCompra").data("kendoComboBox").value(), NombreCamion: $("#InputCamion").val().trim(), token: Cookies.get("token"), Lenguaje: $("#language").val() }).done(function (data) {
        if (Error(data)) {
