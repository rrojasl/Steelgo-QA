...
function AjaxCargarRevision() {
    ...
    $RecepcionOC.RecepcionOC.read({ token: Cookies.get("token"), OrdenCompraID: OrdenCompra }).done(function (data) {
        if (Error(data)) {
...
function AjaxGuardarCaptura(arregloCaptura, tipoGuardar) {
    ...
    for (var i = 0; i < arregloCaptura.length; i++) {
        ...
        Datos[i].CantAP = arregloCaptura[i].CantAP;
        Datos[i].CantRDO = arregloCaptura[i].CantRDO;
        Datos[i].CantCOND = arregloCaptura[i].CantCOND;
        Datos[i].ModificadoPorUsuario = arregloCaptura[i].ModificadoPorUsuario;
    }
    Captura.Detalle = Datos;

    $RecepcionOC.RecepcionOC.create(Captura, { token: Cookies.get("token") }).done(function (data) {
