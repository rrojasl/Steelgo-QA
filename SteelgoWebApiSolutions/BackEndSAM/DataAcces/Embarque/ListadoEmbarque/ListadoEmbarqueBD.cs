                    ...
                    foreach (Sam3_Embarque_LE_Get_ListadoEmbarque_Result item in result)
                    {
                        listaDetalle.Add(new DetalleListadoEmbarque
                        {
                            ...
                            Enviar =
                                ...
                                !item.RequierePapCliente.GetValueOrDefault() && (item.RequiereAduana.GetValueOrDefault() && item.AprobadoAduana == 1) && (item.SolicitudPermiso != null && item.FechaPermiso != null) && item.RequiereRevisionCliente.GetValueOrDefault() && !item.OkClienteEmbarque.GetValueOrDefault() && !item.OkCliente && item.OkEmbarque && item.DestinoID != 0 ? true : //Pesqueria
                                ...
                                item.RequierePapCliente.GetValueOrDefault() && (item.RequiereAduana.GetValueOrDefault() && (item.SolicitudPermiso != null && item.FechaPermiso != null) && item.AprobadoAduana == 1) && !item.RequiereRevisionCliente.GetValueOrDefault() && item.OkClienteEmbarque.GetValueOrDefault() && item.OkCliente && item.OkEmbarque && item.DestinoID != 0 ? true : false, //Salamanca y etileno                            
