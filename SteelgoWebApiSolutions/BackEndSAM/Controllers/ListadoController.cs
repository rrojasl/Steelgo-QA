...
namespace BackEndSAM.Controllers
{
    ...
    public class ListadoController : ApiController
    {
        ...
        public object Get(int tipoListado, int referenciaID, string token)
        {
            string payload = "";
            string newToken = "";
            bool tokenValido = ManageTokens.Instance.ValidateToken(token, out payload, out newToken);
            if (tokenValido)
            {
                ...
                switch (tipoListado)
                {
                    case 28: // Grid incidencias en Pase de salida
                        return ListadoBd.Instance.ListadoIncidenciaPaseSalida(referenciaID, usuario);
                    default:
                        TransactionalInformation result = new TransactionalInformation();
                        result.ReturnMessage.Add("Listado no encontrado");
                        result.ReturnCode = 500;
