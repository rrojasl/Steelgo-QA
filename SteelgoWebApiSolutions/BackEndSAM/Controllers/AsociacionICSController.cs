...
namespace BackEndSAM.Controllers
{
    ...
    public class AsociacionICSController : ApiController
    {
        ...
        public object Delete(string itemCode, string itemCodeSteelgo, string diametro1ID, string diametro2ID, string token)
        {
            string payload = "";
            string newToken = "";
            bool tokenValido = ManageTokens.Instance.ValidateToken(token, out payload, out newToken);
            if (tokenValido)
            {
                ...
                return AsociacionICSBd.Instance.EliminarRelacion(itemCode, itemCodeSteelgo, diametro1ID, diametro2ID, usuario);
            }
            else
            {
                TransactionalInformation result = new TransactionalInformation();
